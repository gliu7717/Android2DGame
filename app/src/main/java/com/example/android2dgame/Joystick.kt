...
class Joystick (centerPositionX: Float,
    ...
    private var acturalX=0f
    private var acturalY=0f
    ...
    {
        innerCircleCenterPositionX = outCircleCenterPositionX +acturalX * outCircleRadius
        innerCircleCenterPositionY = outCircleCenterPositionY +acturalY * outCircleRadius
    }
    ...
    fun setActuator(x: Float, y: Float) {
        ...
        if(deltaDistance < outCircleRadius){
            acturalX = deltaX / outCircleRadius
            acturalY = deltaY / outCircleRadius
        }else{
            acturalX = (deltaX / deltaDistance).toFloat()
            acturalY = (deltaY / deltaDistance).toFloat()
        }
    }
    ...
    fun getAacuatorX(): Float {
        return acturalX
    }
    fun getAacuatorY(): Float {
        return acturalY
    }
